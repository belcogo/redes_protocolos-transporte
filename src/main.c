...
const QUIC_REGISTRATION_CONFIG _registrationConfig = { "quicnetwork", QUIC_EXECUTION_PROFILE_LOW_LATENCY };
const QUIC_BUFFER _alpn = { sizeof("network") - 1, (uint8_t*) "network" };
const uint16_t _port = 4567;
const uint64_t _idleTimeoutMs = 1000;
const uint32_t _bufferLength = 100;
const QUIC_API_TABLE* MsQuic;
HQUIC _registration;
HQUIC _configuration;

void PrintUsageDisclaimer() {
  printf(
    "\n"
    "Usage:\n"
    "\n"
    "./quicsample -client -target:<...> [-unsecure]\n"
    "./quicsample -server -cert_hash:<...> or (-cert_file:<...> and -key_file:<...> (and optionally -password:<...>))\n"
    );
}
...
void ServerSend(_In_ HQUIC Stream) {
  ...
  if (SendBufferRaw == NULL) {
    printf("Alocação do buffer falhou!\n");
  ...
  printf("[strm][%p] Enviando mensagem...\n", Stream);

  QUIC_STATUS Status;
  
  if (QUIC_FAILED(Status = MsQuic->StreamSend(Stream, SendBuffer, 1, QUIC_SEND_FLAG_FIN, SendBuffer))) {
    printf("Falha no envio, 0x%x!\n", Status);
    free(SendBufferRaw);
    MsQuic->StreamShutdown(Stream, QUIC_STREAM_SHUTDOWN_FLAG_ABORT, 0);
  }
}
